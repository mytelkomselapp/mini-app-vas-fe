...
const DetailHadiah = () => {
  ...
  const statusBadgeConfig = {
    pending: {
      ...
      description: 'Barang kamu sedang diproses oleh admin kami. Silakan cek secara berkala untuk dapat informasi pengiriman.'
    },
    failed: {
      text: 'Gagal',
  ...
  return (
    <View className="bg-white min-h-screen">
        ...
        <View className="p-4 bg-white">
          <View className="space-y-2 items-center text-center mb-2">
            ...
            <View className="flex flex-row items-center justify-center w-full">
              <span className={`text-xs leading-[16px] ${status !== 'standby' ? 'font-semibold px-2 py-1 rounded-full' : ''} ${statusConfig.className}`}>
                {statusConfig.text}
              </span>
            </View>

            {status !== 'failed' && <View>
              <View className="h-[1px] bg-dividerGrey w-full" style={{ marginTop: 16, marginBottom: 16 }} />

              <View className="space-y-1 items-center">
                <View className="flex flex-row items-center justify-center text-center mb-2">
                  <Text className="text-xs text-textSecondary">No. Resi:</Text>
                </View>
                {status !== 'standby' && (
                  <Button
                    {...buttonConfig[status]}
      ...
      <View className="mt-2 space-y-2">
        <View className="mx-4 bg-white rounded-lg" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#EFF1F4' }}>
          <View
          ...
          >
            ...
            <Image
              className="w-4 h-4"
              src={ChevronUpIcon}
              style={{ transform: `rotate(${isDetailOpen ? '180deg' : '0deg'})` }}
            />
          </View>
          {isDetailOpen && (
            <View className="px-4 pb-4">
              <Text className="text-sm text-textSecondary">Lorem ipsum dolor sit amet</Text>
            </View>
          )}
        </View>
        ...
        <View className="mx-4 bg-white rounded-lg" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: '#EFF1F4' }}>
          ...
          >
            ...
            <Image
              className="w-4 h-4"
              src={ChevronUpIcon}
              style={{ transform: `rotate(${isTermsOpen ? '180deg' : '0deg'})` }}
            />
          </View>
          {isTermsOpen && (
            <View className="px-4 pb-4">
